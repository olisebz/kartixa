...
export const GET = apiHandler({
  handler: async (_req, ctx) => {
    ...
    const invites = await leagueAccessService.listInviteCodes(ctx.params.leagueId);
    return NextResponse.json(successResponse(invites));
  },
});
...
export const POST = apiHandler<CreateInviteCodeInput>({
  ...
  handler: async (_req, ctx, body) => {
    ...
    const invite = await leagueAccessService.createInviteCode(ctx.params.leagueId, userId, body);
